...
class _ProfilGuruState extends State<ProfilGuru> {
  ...
  Future<void> _ambilData() async {
    try {
      ...
      // Ambil dari tabel kelas
      final kelasData = await supabase
          .from('kelas')
          .select('id, nama_kelas, mata_pelajaran, kode_kelas')
          .eq('guru_id', userId);

      List<Map<String, dynamic>> kelasList = [];
      for (final kelas in (kelasData as List)) {
        // Hitung jumlah murid yang pakai kode kelas ini
        final muridCount = await supabase
            .from('class_members')
            .select('id')
            .eq('mapel_id', kelas['id']);

        kelasList.add({
          'id': kelas['id'],
          'nama': kelas['mata_pelajaran'] ?? '',
      ...
      setState(() {
        _dataGuru = {'nama': userData['nama'] ?? '', 'email': email};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                  ...
                  Stack(
                    ...
                    children: [
                      ...
                      Positioned(
                        top: 16, left: 16,
                        child: IconButton(
                          icon: const Icon(Icons.arrow_back_ios, color: Color(0xFF4A90D9)),
                          onPressed: () => Navigator.pushReplacementNamed(context, '/dashboard-guru'),
                        ),
                      ),
                      Positioned(
                        top: 20,
                        child: Container(
                          ...
                          child: const Icon(Icons.school, size: 50, color: Color(0xFF4A90D9)),
                        ),
                      ),
                      Positioned(
                        bottom: 8,
                          ...
                          children: [
                            Text(_dataGuru?['nama'] ?? '',
                                style: const TextStyle(fontSize: 18,
                                    fontWeight: FontWeight.bold, color: Color(0xFF1A2F5A))),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildMenuButton(
                            label: 'Data Diri',
                            onTap: () => setState(() => _showDataDiri = !_showDataDiri),
                            trailing: Icon(
                              _showDataDiri ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
                              color: Colors.grey,
                            ),
                          ),
                          if (_showDataDiri) ...[
                          ...
                          _buildMenuButton(
                            ...
                            onTap: () => setState(() => _showDaftarKelas = !_showDaftarKelas),
                            trailing: Row(
                              ...
                              children: [
                                if (_kelasList.isNotEmpty)
                                  Container(
                                    ...
                                    child: Text('${_kelasList.length}',
                                        style: const TextStyle(color: Colors.white,
                                            fontSize: 12, fontWeight: FontWeight.bold)),
                                  ),
                                const SizedBox(width: 6),
                                Icon(
                                  _showDaftarKelas ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
                                  color: Colors.grey,
                                ),
                              ],
                            ),
                          ...
                          if (_showDaftarKelas)
                            _kelasList.isEmpty
                                ...
                                : Column(
                                    children: _kelasList.map((kelas) => Container(
                                      margin: const EdgeInsets.only(left: 8, right: 8, bottom: 8),
                                        ...
                                        children: [
                                          Column(
                                            ...
                                            children: [
                                              ...
                                              if ((kelas['kelas'] as String).isNotEmpty)
                                                Text('Kelas ${kelas['kelas']}',
                                          ...
                                          Row(
                                            children: [
                                              const Icon(Icons.people, size: 14, color: Color(0xFF4A90D9)),
                                              const SizedBox(width: 4),
                                              Text('${kelas['jumlah_murid']} murid',
                                                  style: const TextStyle(fontSize: 12,
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (index) {
          switch (index) {
            case 0: Navigator.pushReplacementNamed(context, '/dashboard-guru'); break;
            case 1: Navigator.pushReplacementNamed(context, '/tambah-tugas'); break;
            case 2: Navigator.pushReplacementNamed(context, '/zoom-guru'); break;
            case 3: Navigator.pushReplacementNamed(context, '/kalender-guru'); break;
            case 4: Navigator.pushReplacementNamed(context, '/profil-guru'); break;
          }
        },
      ),
    );
          ...
          children: [
            Row(children: [
              ...
              Text(label, style: const TextStyle(fontSize: 15,
                  fontWeight: FontWeight.w600, color: Color(0xFF1A2F5A))),
            ]),
